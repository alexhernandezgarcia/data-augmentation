    ...
    **kwargs
    ...
    for X_sample, Y_sample in zip(X, Y):
        ...
        for _ in range(n_augmentations):
            ...
            X_oracle_cls, Y_oracle_cls = (
                X_oracle[Y_oracle == Y_sample],
                Y_oracle[Y_oracle == Y_sample],
            )
            diff = np.linalg.norm(
                X_syn - X_oracle_cls, ord=2, axis=1
            )  # Only evaluate norm over samples of same class in the data-bank
            ...
            X_near_noisy = np.array(
                [
                    X_near[0] + np.random.normal(scale=lmd * np.exp(dx_1)),
                    X_near[1] + np.random.normal(scale=lmd * np.exp(dx_2)),
                ]
            ).reshape(-1, 2)
